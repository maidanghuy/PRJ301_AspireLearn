...
public class ListeningServlet extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        // Lấy đường dẫn thực tế trên server nếu file nằm trong thư mục ứng dụng web
        String filePath = getServletContext().getRealPath("/WEB-INF/Excel/" + test.getFilePath().trim());

        int time = test.getDuration();

        ArrayList<Listening> questions = read.getListenning(filePath.trim());

        session.setAttribute("timeTest", time);
        session.setAttribute("listTest", questions);
        session.setAttribute("testID", testID);

        request.getRequestDispatcher("views/user/listening.jsp").forward(request, response);
